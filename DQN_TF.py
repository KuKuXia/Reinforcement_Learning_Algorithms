...
class DeepQNetwork(object):
    def __init__(self, lr, n_actions, name, fc1_dims=1024, input_dims=(210, 160, 4), checkpoint_dir='tmp/dqn'):
        ...
        self.checkpoint_file = os.path.join(
            checkpoint_dir, 'deep_q_network.ckpt')
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            self.q_target = tf.placeholder(
                tf.float32, shape=[None, self.n_actions], name='q_value')

            conv1 = tf.layers.conv2d(inputs=self.input, filters=32, kernel_size=(
                8, 8), strides=4, name='conv1', kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2))
            conv1_activated = tf.nn.relu(conv1)

            conv2 = tf.layers.conv2d(inputs=conv1_activated, filters=64, kernel_size=(
                4, 4), strides=2, name='conv2', kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2))
            conv2_activated = tf.nn.relu(conv2)

            conv3 = tf.layers.conv2d(inputs=conv2_activated, filters=128, kernel_size=(
                3, 3), strides=1, name='conv3', kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2))
            conv3_activated = tf.nn.relu(conv3)

            flatten = tf.layers.flatten(conv3_activated)
            dense1 = tf.layers.dense(flatten, units=self.fc1_dims, activation=tf.nn.relu,
                                     kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2))

            self.Q_values = tf.layers.dense(
                dense1, units=self.n_actions, kernel_initializer=tf.contrib.layers.variance_scaling_initializer(factor=2))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size, replace_target=5000, input_dims=(210, 160, 4),
        ...
        self.q_next = DeepQNetwork(
            alpha, n_actions, input_dims=input_dims, name='q_next', checkpoint_dir=q_next_dir)
        self.q_eval = DeepQNetwork(
            alpha, n_actions, input_dims=input_dims, name='q_eval', checkpoint_dir=q_eval_dir)
    ...
    def choose_action(self, state):
        ...
        if rand < self.epsilon:
            ...
            actions = self.q_eval.session.run(self.q_eval.Q_values, feed_dict={
                                              self.q_eval.input: state})
            action = np.argmax(actions)
        return action

    def learn(self):
        if self.mem_counter % self.replace_target == 0:
            self.update_graph()
        ...
        q_eval = self.q_eval.session.run(self.q_eval.Q_values, feed_dict={
                                         self.q_eval.input: state_batch})

        q_next = self.q_next.session.run(self.q_next.Q_values, feed_dict={
    ...
    def update_graph(self):
        ...
        for t, e in zip(t_params, e_params):
            self.q_eval.session.run(tf.assign(t, e))
