...
class DeepQNetwork(object):
    def __init__(self, lr, n_actions, name, input_dims,
                 fc1_dims=256, fc2_dims=256, checkpoint_dir='tmp/dqn'):
        self.lr = lr
        ...
        self.checkpoint_file = os.path.join(
            checkpoint_dir, 'deep_q_network_tf.ckpt')
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            self.loss = tf.reduce_mean(
                tf.square(self.Q_values - self.q_target))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size,
        ...
        self.q_eval = DeepQNetwork(alpha, n_actions, input_dims=input_dims,
                                   name='q_eval', checkpoint_dir=q_eval_dir)
